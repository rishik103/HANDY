...
public:
  KeyboardTwistTeleop() : Node("keyboard_twist_teleop")
  {
    ...
    declare_parameter("linear_vel", 0.02);
    declare_parameter("angular_vel", 0.1);
    declare_parameter("deadman_timeout_ms", 150);
    ...
    deadman_timeout_ = std::chrono::milliseconds(get_parameter("deadman_timeout_ms").as_int());

    pub_ = create_publisher<geometry_msgs::msg::TwistStamped>(
      "/moveit_servo/delta_twist_cmds", rclcpp::QoS(10).reliable());
    ...
    if (!openKeyboardInput()) {
      ...
      RCLCPP_INFO(get_logger(), "SPACE=stop, ESC/Ctrl+C=exit");
    }
  }

  ~KeyboardTwistTeleop() override
  {
    running_ = false;
...
private:
  ...
  void publishTwist()
  {
...

    {
      ...
      auto now_time = std::chrono::steady_clock::now();
      bool key_valid = key_active_ && (now_time - last_key_time_) <= deadman_timeout_;
  ...
  void enableTwistMode()
  {
    ...
    switch_cmd_client_->async_send_request(
      request,
  ...
  void setTerminalRaw()
  {
    if (tcgetattr(STDIN_FILENO, &orig_term_) != 0) {
      RCLCPP_WARN(get_logger(), "Failed to query terminal attributes");
      return;
    }
    ...
    if (tcsetattr(STDIN_FILENO, TCSANOW, &raw) == 0) {
      ...
      RCLCPP_WARN(get_logger(), "Failed to switch terminal to raw mode");
    }
  }
  ...
  std::chrono::milliseconds deadman_timeout_{150};
